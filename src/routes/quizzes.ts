...
async function findOwnedQuiz(studentId: number, quizId: number, tx: Prisma.TransactionClient = prisma) {
...
async function refreshStudyProgress(tx: Prisma.TransactionClient, studentId: number) {
  const completedQuizzes = await tx.quiz.findMany({
    where: { studentId, state: "COMPLETED", score: { not: null } },
  ...
  return tx.studyProgress.upsert({
    ...
    create: { studentId, completedTopics, totalQuizzes, averageScore },
  });
}
...
export async function quizzesRoutes(app: FastifyInstance) {
  ...
  app.get("/quizzes/:quizId", async (request) => {
    ...
    const quiz = await findOwnedQuiz(student.id, params.quizId);
  ...
  app.post("/quizzes/:quizId/submit", async (request) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const quiz = await findOwnedQuiz(student.id, params.quizId, tx);
      ...
      const studyProgress = await refreshStudyProgress(tx, student.id);
      const completedQuiz = await findOwnedQuiz(student.id, quiz.id, tx);
